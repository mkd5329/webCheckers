...
public class WebServer {
  ...
  public WebServer(final PlayerLobby playerLobby,
    ...
    Objects.requireNonNull(gson, "turnService must not be null");
    //
  ...
  public void initialize() {
    ...
    get(HOME_URL, new GetHomeRoute(playerLobby, templateEngine, checkersGameService));

    // Shows the Checkers game Sign In page.
    get(SIGN_IN_URL, new GetSignInRoute(templateEngine));

    // Sends a request to sign in with a given username.
    post(SIGN_IN_URL, new PostSignInRoute(playerLobby, templateEngine));

    // Sends a request to sign out with a given username.
    post(SIGN_OUT_URL, new PostSignOutRoute(playerLobby, checkersGameService));

    post(HOME_URL, new PostHomeRoute(playerLobby, checkersGameService));

    get(GAME_URL, new GetGameRoute(templateEngine, checkersGameService, gson));

    post(VALIDATE_MOVE_URL, new PostValidateMoveRoute(gson, checkersGameService));

    post(RESIGN_GAME_URL, new PostResignGameRoute(gson, checkersGameService));

    post(CHECK_TURN_URL, new PostCheckTurnRoute(checkersGameService, gson));

    post(SUBMIT_TURN_URL, new PostSubmitTurnRoute(gson, checkersGameService));

    post(BACKUP_MOVE_URL, new PostBackupMoveRoute(gson, checkersGameService));
